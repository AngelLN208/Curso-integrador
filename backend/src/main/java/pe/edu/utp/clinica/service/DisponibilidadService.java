...
public class DisponibilidadService {
        ...
        public List<Map<String, String>> obtenerSlotsDisponibles(Long medicoId, String fechaStr) {
                ...
                // Buscar citas activas (no canceladas) ya registradas ese día
                // para ese médico — mismo criterio que existeConflictoHorario (RF-10)
                LocalDateTime inicioDia = fecha.atStartOfDay();
                LocalDateTime finDia = fecha.atTime(23, 59, 59);
                List<LocalDateTime> citasExistentes = citaRepository
                                .findCitasActivasPorMedicoYRango(medico, inicioDia, finDia)
                                .stream()
                ...
                for (HorarioMedico horario : horariosDia) {
                        ...
                        while (!cursor.isAfter(limite)) {
                                ...
                                boolean ocupado = citasExistentes.stream().anyMatch(
                                                citaHora -> Math.abs(java.time.Duration.between(citaHora, slotDateTime)
