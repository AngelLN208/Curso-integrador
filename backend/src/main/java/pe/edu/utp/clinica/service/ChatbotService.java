...
public class ChatbotService {
        ...
        private ChatbotResponse ejecutarConversacion(ChatbotRequest request, String username) throws Exception {
                ...
                for (int iteracion = 0; iteracion < MAX_TOOL_ITERATIONS; iteracion++) {
                        ...
                        // Defensa 1: la respuesta puede no traer "candidates" en absoluto
                        // (por ejemplo, si el prompt fue bloqueado por promptFeedback).
                        JsonNode candidatos = respuestaGemini.path("candidates");
                        if (!candidatos.isArray() || candidatos.isEmpty()) {
                                log.warn("Gemini no devolvió 'candidates'. promptFeedback: {}",
                                                respuestaGemini.path("promptFeedback"));
                                return respuestaFallback();
                        }

                        JsonNode primerCandidato = candidatos.get(0);
                        String finishReason = primerCandidato.path("finishReason").asText(null);

                        // Defensa 2: el candidato puede no traer "parts" (respuesta cortada
                        // por seguridad, longitud, u otra causa) — antes esto causaba el NPE
                        // "parteRespuesta is null" al llamar parteRespuesta.has(...).
                        JsonNode parteRespuesta = primerCandidato
                                        .path("content")
                                        .path("parts")
                                        .get(0);

                        if (parteRespuesta == null) {
                                log.warn("Gemini devolvió un candidato sin 'parts' (finishReason={})", finishReason);
                                return respuestaFallback();
                        }
                        ...
                        if (parteRespuesta.has("functionCall")) {
                                ...
                                turnoModelo.put("parts", List.of(Map.of(
                                                "functionCall",
                                                objectMapper.convertValue(functionCall, Object.class))));
                                ...
                                turnoResultado.put("parts", List.of(Map.of(
                                                "functionResponse", Map.of(
                                                                "name", nombreFuncion,
                                                                "response", objectMapper.convertValue(resultadoTool,
                                                                                Object.class)))));
                        ...
                        log.warn("Gemini devolvió una parte sin 'text' ni 'functionCall' (finishReason={})",
                                        finishReason);
                        break;
                }
                ...
                return List.of(
                                ...
                                Map.of(
                                                ...
                                                "parameters", Map.of(
                                                                ...
                                                                "required",
                                                                List.of("medicoId", "medicoNombre", "especialidad",
                                                                                "fechaHora"))),
                                Map.of(
